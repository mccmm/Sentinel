...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def import_gobject_from_madcoind(self, madcoind, rec):
        import decimal
        import madcoinlib
        import inflection
        ...
        # shim/madcoind conversion
        object_hex = madcoinlib.SHIM_deserialise_from_madcoind(object_hex)
        objects = madcoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from madcoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from madcoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from madcoind - Madcoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(madcoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from madcoind! %s" % e)
            govobj.vote_delete(madcoind)
            return (govobj, None)
    ...
    def vote_delete(self, madcoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(madcoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def sync_network_vote(self, madcoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = madcoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 madcoin addr, non-multisig
            if not madcoinlib.is_valid_madcoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Madcoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (madcoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/MadcoinDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import madcoinlib
        obj_data = madcoinlib.SHIM_serialise_for_madcoind(self.serialise())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not madcoinlib.is_valid_madcoin_address(addr, config.network):
    ...
    def hash(self):
        import madcoinlib
        return madcoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, madcoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - madcoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, madcoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - madcoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, madcoind):
        now = int(time.time())
        return (self.created_at < (now - madcoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, madcoind):
        if self.is_expired(madcoind):
            return False

        return True

    def is_deletable(self, madcoind):
        if self.is_expired(madcoind):
            return True
