...
def has_madcoin_conf():
    import config
    import io

    valid_madcoin_conf = False

    # ensure madcoin_conf exists & readable
    #
    # if not, print a message stating that Madcoin Core must be installed and
    # configured, including JSONRPC access in madcoin.conf
    try:
        f = io.open(config.madcoin_conf)
        valid_madcoin_conf = True
    except IOError as e:
        print(e)

    return valid_madcoin_conf
...
def main():
    ...
    if not has_madcoin_conf():
        print("MadcoinCore must be installed and configured, including JSONRPC access in madcoin.conf")
