...
sentinel_cfg = MadcoinConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_madcoind_proto_version_with_sentinel_ping = 70207
...
def get_madcoin_conf():
    args = get_args()

    if args.config:
        madcoin_conf = args.config
    else:
        home = os.environ.get('HOME')
        if home is not None:
            if sys.platform == 'darwin':
                madcoin_conf = os.path.join(home, "Library/Application Support/MadcoinCore/madcoin.conf")
            else:
                madcoin_conf = os.path.join(home, ".madcoincore/madcoin.conf")
        else:
            home = os.getenv('APPDATA')
            if home is not None:
                madcoin_conf = os.path.join(home, "madcoincore\\madcoin.conf")
            else:
                madcoin_conf = 'madcoin.conf'
        
        madcoin_conf = sentinel_cfg.get('madcoin_conf', madcoin_conf)
