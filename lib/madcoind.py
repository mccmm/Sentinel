...
class MadcoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some madcoind methods
    ...
    @classmethod
    def from_madcoin_conf(self, madcoin_dot_conf):
        from madcoin_config import MadcoinConfig
        config_text = MadcoinConfig.slurp_config_file(madcoin_dot_conf)
        creds = MadcoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = madcoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = madcoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = madcoinlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_madcoind_proto_version_with_sentinel_ping)
