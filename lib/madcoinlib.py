...
def is_valid_madcoin_address(address, network='mainnet'):
    ...
    madcoin_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Madcoin address
        return False

    if (address_version != madcoin_version):
        return False
...
def SHIM_serialise_for_madcoind(sentinel_hex):
    ...
    # madcoind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    madcoind_hex = serialise(obj)
    return madcoind_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('madcoin.conf')
